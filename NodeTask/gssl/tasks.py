...
def evaluate_node_classification_acc_linearprob(
    z: torch.Tensor,
    data: Data,
    masks: Dict[str, torch.Tensor],
    fix_linear: int = 0,
    ...
    acc_val, acc_test = train_pytorch_modelFix(
        emb_dim=z.shape[1],
        num_cls=num_cls,
        X=z,
        y=y,
...
def train_pytorch_model(
    ...
    for weight_decay in pbar:
        ...
        acc = sk_mtr.classification_report(
            y_true=y[masks["val"]].cpu().numpy(),
            y_pred=lr_model.predict(X[masks["val"]]).cpu().numpy(),
            output_dict=True,
            zero_division=0,
        )["accuracy"]
...
def train_pytorch_modelFix(
    ...
    best_val, best_test = lr_model.train_fit(X, y, masks)
...
class LogisticRegression(nn.Module):
    ...
    def acc(self, pred, ground):
        accs = (pred.argmax(-1) == ground).sum()/pred.shape[0]
        return accs
    ...
    def train_fit(self, X, y, split, tune_path):
        self.train()
        X = X.to(self._device)
        y = y.to(self._device)
        best_val = 0
        ...
        for _ in tqdm(range(3000), desc="Epochs", leave=False):
            self._optimizer.zero_grad()

            pred = self(X)
            loss = self._loss_fn(input=pred[split['train']], target=y[split['train']])
            acc_val = self.acc(pred[split['val']], y[split['val']])
            acc_test = self.acc(pred[split['test']], y[split['test']])
            if acc_val > best_val:
                best_val = acc_val
                best_test = acc_test
                torch.save(self.state_dict(), tune_path)
