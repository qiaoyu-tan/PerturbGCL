...
from gssl.tasks import evaluate_node_classification_acc_linearprob
...
class BGRL(nn.Module):
    ...
    def predicttoy(self, x, edge_index):
        with torch.no_grad():
            return self._online_encoder(x, edge_index).cpu()


class BGRLTune(nn.Module):
    def __init__(
        self,
        encoder: nn.Module,
        augmentor: GraphAugmentor,
    ...
    def forward(self, data: Data):
        ...
        with torch.no_grad():
            h1_target = self._target_encoder(x1, edge_index1)
            h2_target = self._target_encoder(x2, edge_index2)
...
def test(
    ...
    accs = evaluate_node_classification_acc_linearprob(
        z=z, data=data, masks=masks, use_pytorch=use_pytorch_eval_model, fix_linear=fix_linear,
    )
