...
class PredictionModelNew(nn.Module):

    def __init__(self, encoder, pred_head, dim, out_dim, target_k=2):

        super(PredictionModelNew, self).__init__()
        self.encoder = encoder
    ...
    def forward(self, data):
        ...
        zg = self.encoder(data, self.target_k, finetune=True)
        out = self.pred_head(zg)
...
class Eval_unsuper(object):
    ...
    def evaluate(self, model, fold_seed=10, target_k=2):
        r"""Run evaluation with given learning model and encoder(s).
        ...
        embed, lbls = self.get_embedNew(model, loader, target_k)
    ...
    def get_embed(self, model, loader, target_k=2):
    ...
    def get_embedNew(self, model, loader, target_k=2):
        ...
        with torch.no_grad():
            for data in loader:
                y.append(data.y.numpy())
                data.to(self.device)
                embed = model.encode_view(data, target_k, finetune=True)
                ret.append(embed.cpu().numpy())
...
class Eval_semi(object):
    ...
    def evaluate(self, pre_model, pred_head=None, fold_seed=12345, target_k=2):
        r"""Run evaluation with given learning model and encoder(s).
        ...
        :rtype: (float, float)
        """
        model = PredictionModelNew(pre_model.encoder, pred_head, pre_model.d_out, self.out_dim, target_k).to(self.device)
    ...
    def grid_search(self, learning_model, pred_head=None, fold_seed=12345,
                    p_lr_lst=[0.1, 0.01, 0.001, 0.0001], p_epoch_lst=[20, 40, 60, 80, 100], target_k=2):
        ...
        for p_lr in p_lr_lst:
            for p_epoch in p_epoch_lst:
                ...
                acc_m, acc_sd = self.evaluate(learning_model, pred_head, fold_seed, target_k)
                acc_m_lst.append(acc_m)
                acc_sd_lst.append(acc_sd)
                paras.append((p_lr, p_epoch))
        idx = np.argmax(acc_m_lst)
        print('Best paras: %d epoch, lr=%f, acc=%.4f, std=%.4f' % (
            paras[idx][1], paras[idx][0], acc_m_lst[idx], acc_sd_lst[idx]))
